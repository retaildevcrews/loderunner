...
namespace LodeRunner.API.Test.IntegrationTests.Controllers
{
    ...
    public class TestRuns : IClassFixture<ApiWebApplicationFactory<Startup>>
    {
        ...
        private async Task CanGetClientsById()
        {
            ...
            await httpClient.WaitAndValidateGetByIdToMatchStatus(TestRunsUri, clientStatusId, ClientStatusType.Ready, this.jsonOptions, this.output);

            l8rService.StopService();

            await httpClient.WaitAndValidateGetByIdToMatchStatus(TestRunsUri, clientStatusId, ClientStatusType.Terminating, this.jsonOptions, this.output);
        }
    }
}
