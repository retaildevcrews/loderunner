        ...
        public async Task CanCreateAndExecuteTestRun(int apiHostCount)
        {
            ...
            try
            {
                ...
                foreach (var (hostId, portNumber, apiProcessContext) in apiProcessContextCollection)
                {
                    ...
                    var parentSpanId = await this.TryParseProcessOutputAndGetValueFromFieldName(apiProcessContext.Output, LodeRunner.Core.SystemConstants.LodeRunnerAPIRequestLogName, lodeRunnerAPIOutputMarker, LodeRunner.Core.SystemConstants.B3ParentSpanIdFieldName, 10, 500);
