...
| --help (-h)            | N/A                  | none             | null     | Display LodeRunner command line options.  | If passed all other parameters are ignored. |
...
| --region               | REGION               | string           | null | Deployment region for logging (user defined). |            |
...
| --server (-s)          | SERVER               | string1 [string2 string3 ...] | null   | Server URL(s) to test (i.e. `https://MyServerDomainName.com`). |            |
...
| --zone                 | ZONE                 | string           | null | Deployment zone for logging (user defined). |            |
