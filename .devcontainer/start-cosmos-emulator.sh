...
docker run -itd --restart always -v "${EMUL_DATA_PATH}":/tmp/cosmos/appdata -p 9090:8081 -p 10251:10251 -p 10252:10252 -p 10253:10253 -p 10254:10254 -m 3g --cpus=2.0 --name=${EMUL_NAME} -e AZURE_COSMOS_EMULATOR_PARTITION_COUNT=10 -e AZURE_COSMOS_EMULATOR_ENABLE_DATA_PERSISTENCE=true -e AZURE_COSMOS_EMULATOR_IP_ADDRESS_OVERRIDE=${ipaddr}  mcr.microsoft.com/cosmosdb/linux/azure-cosmos-emulator
...
sudo echo "127.0.0.1  ${EMUL_URL}" > /etc/hosts

## Generate keys for NGINX
openssl req -x509 -newkey rsa:4096 -sha256 -days 3650 -nodes \
  -keyout ${NGINX_CONF_PATH}/nginx.key -out ${NGINX_CONF_PATH}/nginx.crt -subj "/CN=${EMUL_URL}" \
  -addext "subjectAltName=DNS:${EMUL_URL},DNS:${EMUL_URL},IP:10.0.0.1"
